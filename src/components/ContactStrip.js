...
export default function ContactStrip() {
  return (
    <div
      className="flex flex-wrap items-center justify-center gap-x-8 gap-y-2 px-5 sm:px-8 py-2.5 text-[0.9rem] font-semibold"
      style={{ backgroundColor: BG_COLOR }}
    >
      {/* Group 1: Phone + WhatsApp stay on the same line, shrink on small screens */}
      <div className="flex flex-nowrap items-center gap-x-4 xs:gap-x-5 sm:gap-x-6 text-[0.6rem] xs:text-[0.7rem] sm:text-[0.85rem] md:text-[0.9rem]">
        <a
          href="tel:+447414270363"
          aria-label="Call UK number"
          className="inline-flex items-center gap-1.5 sm:gap-2 whitespace-nowrap no-underline transition-opacity hover:underline hover:opacity-70"
      ...
      <a
        ...
        aria-label="Email us"
        className="inline-flex items-center gap-2 whitespace-nowrap no-underline transition-opacity hover:underline hover:opacity-70 text-[0.75rem] sm:text-[0.9rem]"
        style={{ color: TEXT_COLOR }}
      >
        <MailIcon />
...
function PhoneIcon() {
  return (
    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={TEXT_COLOR} strokeWidth="2" className="shrink-0 sm:w-4 sm:h-4">
      <path
...
function WhatsAppIcon() {
  return (
    <svg width="14" height="14" viewBox="0 0 24 24" fill={TEXT_COLOR} className="shrink-0 sm:w-4 sm:h-4">
